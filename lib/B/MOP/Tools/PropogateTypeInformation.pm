...
class B::MOP::Tools::PropogateTypeInformation {
    ...
    method visit_subroutine ($node) {
        # TODO: set the type of the subroutine ...
    }

    method visit_block     ($node) { $node->set_type($node->statements->[-1]->get_type) }
    method visit_statement ($node) { $node->set_type($node->expression->get_type)       }
    ...
    method visit_local_fetch ($node) {
        ...
        #    $self->_cast_node_type($node, $pad_type)
    ...
    method visit_local_store ($node) {
        ...
        $node_type->cast_type_into($target_type);
    }
    ...
    method _cast_pad_target ($pad_target, $type) {
        my $pad_type = $pad_target->get_type;
        return $pad_type if $pad_type->is_same_type($type);

        my $new_type = $pad_type->cast($type);
        return unless $new_type;

        $pad_target->set_type($new_type);
        return $new_type;
    }

    method _cast_node_type ($node, $type) {
        my $node_type = $node->get_type;
        return $node_type if $node_type->is_same_type($type);

        my $new_type = $node_type->cast($type);
        return unless $new_type;
...
__END__
        ...
        if ($lhs_type->is_same_type($rhs_type)) {
            ...
            if ($node_type->is_same_type($lhs_type)) {
                ...
                # protogate to the pad
                # NOTE: might need to cast here, to preserve type changes
                $node->lhs->get_target->set_type($node->lhs->get_type);
                $node->rhs->get_target->set_type($node->rhs->get_type);
